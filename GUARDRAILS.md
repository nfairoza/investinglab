...
| Power Trades Influence Context (FEC) | Campaign-finance — **NOT trades**. Stored in `power_influence_records` (separate table), shown ONLY in the Influence Context tab, never in the Alpha Feed, never scored, never styled buy/sell. Uses aggregation endpoints only — NO individual donor street addresses. FEC contributor lists are non-commercial use only. Off until `POWER_TRADES_ENABLE_FEC` + `FEC_API_KEY`. **OpenSecrets public API was discontinued 2025-04-15** — its adapter is permanently disabled (`built:false`), never fabricates data; reviving needs a commercial agreement. | `lib/power-trades/influence.ts`, `/api/power-trades/influence`, `components/power-trades/influence-context.tsx` |
