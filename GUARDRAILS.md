...
| Power Trades Influence Context (FEC + OpenSecrets) | Campaign-finance + lobbying — **NOT trades**. Stored in `power_influence_records` (separate table), shown ONLY in the Influence Context tab, never in the Alpha Feed, never scored, never styled buy/sell. Uses aggregation endpoints only — NO individual donor street addresses. OpenSecrets attributed (CC BY-NC-SA); both non-commercial. Off until `POWER_TRADES_ENABLE_FEC` / `POWER_TRADES_ENABLE_OPENSECRETS` + keys. | `lib/power-trades/influence.ts`, `/api/power-trades/influence`, `components/power-trades/influence-context.tsx` |
