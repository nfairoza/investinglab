...
function tone(score: number): string {
  ...
  if (score >= 45) return "text-slate-200";
  if (score >= 30) return "text-amber-300";
  return "text-rose-300";
}
function barColor(score: number): string {
  if (score >= 60) return "bg-emerald-500";
  if (score >= 45) return "bg-slate-400";
...
export function ScoreCard({ symbol }: { symbol: string }) {
  ...
  return (
    <div className="card-hover rounded-xl border border-slate-800 bg-slate-900/40 p-5">
            ...
            {(Object.keys(HORIZON_LABELS) as Horizon[]).map((h) => {
              ...
              return (
                <div key={h} className="flex items-center gap-3">
                  <span className="w-16 shrink-0 text-xs text-slate-500">{HORIZON_LABELS[h]}</span>
                  <div className="h-2 flex-1 overflow-hidden rounded bg-slate-800">
                    {v != null && <div className={`h-full ${barColor(v)}`} style={{ width: `${v}%` }} />}
                  </div>
                  <span className="w-10 shrink-0 text-right text-xs text-slate-400">{v != null ? Math.round(v) : "—"}</span>
            ...
            <div className="space-y-1.5">
              {s.factors.map((f) => (
                <div key={f.id} className="flex items-center gap-3 text-sm">
                  <span className="w-36 shrink-0 text-slate-400">{f.label}</span>
                  ...
                  <span className="w-8 shrink-0 text-right text-xs text-slate-500">{f.available && f.score != null ? Math.round(f.score) : "—"}</span>
                  <span className="hidden w-64 shrink-0 truncate text-xs text-slate-600 md:block" title={f.detail}>{f.detail}</span>
                </div>
              ))}
            </div>
      ...
      <p className="mt-3 text-[11px] text-slate-600">Research and educational analysis, not financial advice.</p>
    </div>
  );
}
