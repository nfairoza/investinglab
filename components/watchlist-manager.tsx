...
import { useLocalList, newId, type WatchItem } from "@/lib/local-store";
...
export function WatchlistManager() {
  const { items, ready, add, remove } = useLocalList<WatchItem>("stockpilot.watchlist");
  ...
  function addItem() {
    ...
    add({ id: newId(), symbol: sym, idealBuy: Number.isFinite(target) && target > 0 ? target : undefined, note: note.trim() || undefined });
    setSymbol("");
    setIdealBuy("");
    setNote("");
  }
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <input value={symbol} onChange={(e) => setSymbol(e.target.value)} placeholder="Ticker (e.g. NVDA)"
          ...
          <input value={idealBuy} onChange={(e) => setIdealBuy(e.target.value)} placeholder="Ideal buy $ (optional)" inputMode="decimal"
            className="rounded-md border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-200 placeholder:text-slate-600 focus:border-brand-500 focus:outline-none" />
          <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (optional)"
            className="rounded-md border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-200 placeholder:text-slate-600 focus:border-brand-500 focus:outline-none" />
          <button onClick={addItem} className="rounded-md bg-brand-600 px-3 py-2 text-sm font-medium text-white hover:bg-brand-600">
            Add to watchlist
          </button>
        </div>
      </div>
                ...
                {items.map((w) => {
                  ...
                  return (
                    <tr key={w.id} className="hover:bg-slate-800/30">
                      <td className="px-3 py-2 font-medium text-slate-200">{w.symbol}</td>
                      ...
                      <td className="px-3 py-2">
                        {atOrBelow == null ? (
                          ...
                          <span className="text-emerald-400">● at/below target</span>
                        ) : (
                      ...
                      <td className="px-3 py-2 text-right">
                        <button onClick={() => remove(w.id)} className="text-xs text-slate-500 hover:text-rose-300">Remove</button>
                      </td>
                    </tr>
                  );
                })}
