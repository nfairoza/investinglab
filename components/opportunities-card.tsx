...
import { Sparkles, TrendingUp, TrendingDown, RefreshCw, ArrowRight } from "lucide-react";
...
export function OpportunitiesCard() {
  ...
  // On mount: load cache; if fresh use it, else auto-run once.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (j.data) { setResult(j.data); setCached(true); } // show stale while refreshing
        run(); // generate fresh
      } catch {
        run();
      }
