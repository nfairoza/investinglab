...
//
// Per-user: credentials are passed in (from the user's broker_connections row),
// never read from a shared global.
// =============================================================================

const BASE = "https://trading.robinhood.com";

export interface RhCryptoCreds {
  apiKey: string;
  privateKey: string; // base64 seed
}

export function hasCryptoCreds(creds: RhCryptoCreds | null | undefined): boolean {
  return Boolean(creds?.apiKey && creds?.privateKey);
}
...
function loadPrivateKey(privateKeyB64: string): crypto.KeyObject {
...
async function signedGet<T>(creds: RhCryptoCreds, path: string): Promise<T> {
  if (!hasCryptoCreds(creds)) throw new Error("Robinhood crypto keys not set");
  const ts = Math.floor(Date.now() / 1000).toString();
  const method = "GET";
  const body = ""; // GET has no body
  const message = `${creds.apiKey}${ts}${path}${method}${body}`;
  const signature = crypto.sign(null, Buffer.from(message), loadPrivateKey(creds.privateKey)).toString("base64");
...
export async function getCryptoHoldings(creds: RhCryptoCreds): Promise<RhCryptoHolding[]> {
  const out: RhCryptoHolding[] = [];
  let path = "/api/v1/crypto/trading/holdings/";
  for (let guard = 0; guard < 10; guard++) {
    const data = await signedGet<any>(creds, path);
...
export async function getCryptoPrice(creds: RhCryptoCreds, symbol: string): Promise<number | null> {
  try {
    const data = await signedGet<any>(creds, `/api/v1/crypto/marketdata/best_bid_ask/?symbol=${symbol}-USD`);
