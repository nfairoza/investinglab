...
export async function GET() {
  ...
  // selectPlaidItems degrades gracefully if the enc columns aren't migrated yet,
  // and surfaces a REAL db error instead of the connect-brokerage empty state.
  ...
  const results = await Promise.allSettled(
    items.map((it) => {
      ...
      return plaid.investmentsHoldingsGet({ access_token: token });
    }),
  );
